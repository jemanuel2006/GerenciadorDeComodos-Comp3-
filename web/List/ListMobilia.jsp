...
<body>
    ...
    <div class="container">
        <legend>Cozinhas Disponíveis</legend>
        <button class="btn btn-success" id="btnCreate"><span class="glyphicon glyphicon-plus"></span> Criar Cozinha</button>
    ...
    <script type="application/javascript">
        $("#btnCreate").on("click", function(){
            window.location.href = "${basePath}" + "/Create/Cozinha";
        });
            ...
            fields: [
                {
                    ...
                    itemTemplate: function(value, item) {
                        return $("<button class='jsgrid-button jsgrid-edit-button'>")
                                .on("click", function () {
                                    window.location.href = "${basePath}" + "/Edit/Cozinha?id=" + item.Id;
                                });
                    }
                },
                ...
                { name: "Descrição", type: "text", width: 225, validate: "required" },
                {
                    ...
                    itemTemplate: function(value, item) {
                        return $("<button class='jsgrid-button jsgrid-delete-button'>")
                                .on("click", function () {
                                    swal({
                                        title: "Deseja apagar a cozinha '" + item.Descrição + "'?",
                                        ...
                                        if (isConfirm) {
                                            $.ajax({
                                                type: "DELETE",
                                                url: "/Delete/Cozinha?id=" + item.Id
                                            })
                                            .done(function(data) {
                                                swal("Cozinha apagada!", "A cozinha ''" + item.Descrição + "' foi apagada.", "success");
