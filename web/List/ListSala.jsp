...
<body>
    ...
    <div class="container">
        ...
        <button class="btn btn-success" id="btnCreate"><span class="glyphicon glyphicon-plus"></span> Criar Sala</button>
    ...
    <script type="application/javascript">
        $("#btnCreate").on("click", function(){
            window.location.href = "${basePath}" + "/Create/Sala";
        });
            ...
            fields: [
                {
                    ...
                    itemTemplate: function(value, item) {
                        return $("<button class='jsgrid-button jsgrid-edit-button'>")
                                .on("click", function () {
                                    window.location.href = "${basePath}" + "/Edit/Sala?id=" + item.Id;
                                });
                    }
                },
                ...
                { name: "Descrição", type: "text", width: 225, validate: "required" },
                {
                    ...
                    itemTemplate: function(value, item) {
                        return $("<button class='jsgrid-button jsgrid-delete-button'>")
                                .on("click", function () {
                                    swal({
                                        title: "Deseja apagar a sala '" + item.Descrição + "'?",
                                        ...
                                        if (isConfirm) {
                                            $.ajax({
                                                type: "DELETE",
                                                url: "/Delete/Sala?id=" + item.Id
                                            })
                                            .done(function(data) {
                                                swal("Sala apagada!", "A sala ''" + item.Descrição + "' foi apagada.", "success");
                                                $("#grid").jsGrid("deleteItem", item);
                                            })
                                            .fail(function() {
                                                swal("Erro", "Ocorreu um erro ao apagar a sala ''" + item.Descrição +
